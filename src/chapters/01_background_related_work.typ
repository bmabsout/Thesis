...
The key property is that the transition function $TT$ is only defined with respect to the current #state(state) and #action(action), with no _knowledge_ of the history of transitions. This property enables many of the techniques employed in #abbrv.RL.
...
==== Rewards <def:reward>
  The reward function $R : #S times #A times #S -> RR$ is a function that maps a transition to a real-valued reward, in #abbrv.RL it is our main interface for choosing which policies are better at solving the task at hand than others.
  #figure(
...
Established in the foundational #abbrv.RL textbook by @SuttonBarto the hypothesis states: "all of what we mean by goals and purposes can be well thought of as maximization of the expected value of the cumulative sum of a received scalar signal." This hypothesis has provided a unifying framework for #abbrv.RL research, enabling the development of powerful algorithms like Q-learning, policy gradients, and actor-critic methods.


==== Returns <def:return>
The return is the discounted sum of rewards: $sum_(t=0)^infinity gamma^t rt$. The discount factor $gamma$ determines how much to prioritize immediate versus future rewards. Mapping a trajectory to a value in $RR$. The main goal in #abbrv.RL is to find policies that maximize the expected return. 
...
Value functions form the basis of deep #abbrv.RL @DQN_paper, capturing the specific notion of optimality emerging from the chosen reward function. They are commonly approximated by neural networks.
...
The most prevalent utility function in #abbrv.MORL based methods is the linear utility function, which combines objectives as weighted sums: $u_("linear")(bold(arrow(G))) = sum_(i=1)^n w_i G_i$. Using this approach, we can define flattened Q-value functions as follows:
...
#abbrv.MORL adapts the foundational concept of Pareto optimality to policy space. A policy $pi^*$ is Pareto optimal if no other policy $pi'$ exists such that $reward(V_i)^(pi')(state(s)) >= reward(V_i)^(pi^*)(state(s))$ for all objectives $i$ and states $s$, with strict inequality for at least one objective-state pair. The Pareto frontier represents the set of all Pareto optimal policies, providing a complete characterization of optimal trade-offs in policy space.
...
+ *#abbrv.BC* @pomerleau1989alvinn completely avoids reward engineering by framing policy learning as supervised learning, directly mapping observed states to expert actions with no reward signal.  This simplicity is attractive, but #abbrv.BC suffers from covariate-shift: once the learned policy visits states absent from the demonstrations, errors can compound.

+ *#abbrv.IRL* avoids manual reward engineering by automatically inferring reward functions from expert demonstrations @inverse_rl_survey rather than requiring explicit design. The work of @ng2000algorithms established the theoretical foundations, while @abbeel2004apprenticeship demonstrated practical applications. However, #abbrv.IRL approaches still typically result in scalar reward functions and face challenges in multi-objective scenarios where expert demonstrations may represent complex trade-offs between competing objectives.

==== Automating Specification with Large Language Models <rel:llm_spec>
Methods such as Eureka @eureka and other similar approaches @yu2023language make use of an #abbrv.LLM to emit reward code, using a vision model to evaluate the adherence of the final policy to the original intentions representing in the textual description. This reduces the *manual* burden of writing and iteratively checking reward code after every run, but it merely relocates the specification problem: now the *#abbrv.LLM* must itself carry out the reward-engineering step, inheriting the same structural difficulties. Because this translation is intrinsically hard, success is unpredictable—some tasks work zero-shot, others fail entirely—so the fundamental challenge remains.

==== Multi-Objective Reinforcement Learning <rel:morl>
#abbrv.MORL aims to solve problems with multiple, often competing, objectives. The dominant paradigm, known as _a-posteriori_ #abbrv.MORL, focuses on learning a Pareto front of optimal trade-offs, from which a user can later select a policy.
...
Replay-based rehearsal, elastic weight consolidation and regularisation methods slow forgetting @catastrophic-forgetting-wolczyk, but still require careful task sequencing. Conservative policy optimisation bounds performance drops via constraints.

=== Sample Efficiency <chap:background:related:sample_eff>
Modern model-free #abbrv.RL (e.g. #abbrv.SAC) needs millions of transitions, prohibitive for on-hardware learning.

Existing solutions fall into three categories:
Ensemble critic methods such as REDQ by @REDQ and TQC by @TQC, which reduce variance, model-based methods (DreamerV3, MuZero), and tweaks to architecture as is the case for CrossQ by @CrossQ. 
