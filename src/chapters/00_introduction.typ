...
+ *Sample inefficiency*: popular #abbrv.RL algorithms are sample-hungry, requiring lengthy, expensive data collection #changed[@sample_efficient_rl].
...
Reasoning across the entire stack, from user intention to careful treatment of the embedded systems running on-robot, is essential for bridging the intent-to-reality gap. This wholistic approach enabled the achievement of #changed[several improvements over state-of-the-art results]: the first #abbrv.RL attitude controller to outperform classical #abbrv.PID controllers on real racing quadrotors, reductions in power consumption by 50-80% compared to Neuroflight as well as smoothness improvements accross Gymnasium benchmarks, live and robust neural network adaptation, and consistent policy search with up to $6.4 times$ better sample efficiency than baseline methods across various robotic domains.
...
*Sample Efficiency*: Up to 6.4$times$ faster learning than #abbrv.DDPG on LunarLanderContinuous-v2 and 5.6$times$ faster on Hopper-v4 using our Balanced Policy Gradient algorithm
...
*Energy Consumption*: Almost 80% power reductions in quadrotor control demonstrated through #abbrv.CAPS regularization
