...
With advances in real-time cyber-physical systems we can now run sophisticated neural network controllers entirely aboard mobile robots. Yet when practitioners train #abbrv.RL policies destined for real-world deployment they stumble on four recurring problems: (1) scalar rewards are notoriously hard to design; (2) policies that excel in simulation often fail on real robots, causing crashes and oscillations among other issues; (3) fine-tuning on real data causes catastrophic forgetting; and (4) popular #abbrv.RL algorithms remain sample-inefficient. These shortcomings are reflected in a wide "intent-to-reality gap"---the discrepancy between designer intent and deployed robot behaviour.
...
embedded stack, we achieve: (i) the first #abbrv.RL attitude controller to 
