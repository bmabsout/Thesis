...
== Defining Behavior: Trajectory Sets and Enrichments
...
#note(gradient: primary_gradient, title: [=== EOE Operators <eoe_operators>])[ 
  #table(
...
=== Instantiating EOE with Reinforcement Learning
...
=== Instantiating EOE with Imitation Learning
...
=== The Intent-to-Specification Gap <gap:specification>
...
=== The Specification-to-Behavior Gap <gap:optimization>
...
*Sources of the Gap*:
+ *Optimization Pathologies*: The optimization landscape may be fraught with poor local minima, causing optimization algorithms to fail to find policies that are anywhere near optimality with respect to the specification `spec`. For example, deep learning-based value iteration methods are notoriously difficult to stabilize, often converging to policies that are far from what the specification describes.
...
=== The Simulation-to-Reality Gap <gap:reality>
