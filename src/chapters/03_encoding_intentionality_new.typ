...
=== Formalizing Intuitive Judgment
...
=== Important Properties for a Logic of Differentiable Objectives
...
=== The Generalized Mean As a Logic Operator
...
=== Formal Definition: Syntax and Semantics
==== Syntax
...
For conjunctive compositions, FPL provides a powerful guarantee about the minimum level of fulfillment for all objectives.
...
==== Lexicographical-Style Priorities
...
=== Semantic Calibration vs. Compositional Priority <semantic-vs-compositional>
...
==== Semantic Calibration of Individual Fulfillments
...
==== Compositional Priority via Offset
...
==== Orthogonal Mechanisms in Practice
...
== Building an Optimizer for FPL Specifications
...
=== fulfillment Q-Values ($f$Q-Values)
...
$ f"Q"_i^(pi)(s,a) = E_pi [sum_(t=0)^infinity gamma^t f_(i)(s_t, a_t) | s_0=s, a_0=a] $
where $f_(i)(s_t, a_t)$ is the immediate fulfillment reward from the state-action pair at time $t$.

Since fulfillment rewards are in $[0,1]$, the raw $f$Q-value lies in $[0, 1/(1-gamma)]$. To make $f$Q-values compatible with FPL, which operates on $[0,1]$, we normalize them:
$ hat(f"Q")_i^(pi)(s,a) = f"Q"_i^(pi)(s,a) (1-gamma) $

This normalized $f$Q-value, $hat(f"Q")_i^pi$, represents the average per-timestep fulfillment for objective $i$ and can be used directly as a base fulfillment value in an FPL formula.

=== The Balanced Policy Gradient (BPG) Algorithm
...
#algorithm(title: [Balanced Policy Gradient (BPG)])[
  ...
  $
  f"Q"_(phi)(s,a) = u_(phi)(hat(bold(f"Q"))(s,a))
  $

  + *Actor Update*: The deterministic actor is updated using the policy gradient of the batch's mean composed $f$Q-value, $J$:
  $
  J = pmean(2)(u_(phi)(hat(bold(f"Q"))(s, pi_theta(s))))
  $
]
...
$
L_"TD" = pmean(2)(bold(y)^"TD" - bold(f"Q")(s, a))
$
...
$
L_(f"V") = pmean(2)(f"V"_"obs" - bold(f"Q")(s, a))
$
...
=== Behavioral Analysis: Preventing Reward Hacking
...
=== Phase 1: Capturing Intent for a Single Objective
...
==== Step 1: Define the Semantic Mapping
...
==== Step 2: Implement the fulfillment Reward Function
Translate your semantic map into a function $f_i(s,a,s') -> [0,1]$. This function should be stateless and only consider the information within a single transition.

==== Step 3: Validate with a Single-Objective Agent
...
==== Step 2: Construct the FPL Formula
...
==== Step 3: Train the Multi-Objective Agent
...
==== Step 4: Debug via Interpretable Components
...
=== Robustifications of Existing Logics
